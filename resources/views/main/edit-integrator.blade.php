            ...
            <div class="col-6 mx-auto mt-5">
                <div class="card shadow rounded">
                    ...
                    <div class="card-body new-color">
                        ...
                        @php
                            $key = Hash::make($integrator->id);
                        @endphp
                        <form action="{{ route("updateIntegrator",["id"=>$integrator->id]) }}" method="post">
                            @csrf
                            @method('put')
                            <div class="mb-3">
                                <label for="exampleFormControlInput1" class="form-label">الاسم</label>
                                <input name="name" type="text" value="{{ $integrator->name }}" class="form-control  @error('name') is-invalid  @enderror" id="exampleFormControlInput1" placeholder="البريد الالكتروني">
                                @error('name')
